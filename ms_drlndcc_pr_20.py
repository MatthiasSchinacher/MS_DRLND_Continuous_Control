...
rl.write('#\n# Episode Score average(last-100-Scores) MinScore MaxScore RMSize Epsilon\n')
train_mode = not SHOW

for episode in range(1,EPISODES+1):
    env_info = env.reset(train_mode=train_mode)[brain_name] # reset the environment
    state  = env_info.vector_observations               # get the start state
    score  = [0.0 for z in range(num_agents)]           # initialize the episode- score
    ...
    while True:
        ...
        if TRAIN and episode <= WARMUP_EPISODES:
            ...
            with torch.no_grad():
                for z in range(num_agents):
                    tmp = torch.unsqueeze(torch.tensor(state[z]),0)
                    action[z] = np.resize(modelA(tmp).detach().numpy(),(ACTION_SIZE,))
            modelA.train()

            noise += -NOISE_THETA * noise + NOISE_SIGMA * np.random.rand(num_agents,4)
        ...
        for z in range(num_agents):
            reward = env_info.rewards[z]             # get the reward
            fr = float(reward)
            done = env_info.local_done[z]            # see if episode has finished
            score[z] += fr                           # update the score
            ...
            if TRAIN:
                ...
                if fr > 0.0 or float(np.random.random_sample()) < NO_REWARD_RM_PROB:
                    ...
                    transition = (state[z],action[z],reward,next_state[z],done)
    ...
    avscore  = float(sum(score))/float(len(score))
    minscore = float(min(score))
    maxscore = float(max(score))
    
    score_buffer.append(avscore)
    while len(score_buffer) > 100:
        score_buffer.pop(0)
    
    l100_score = float(sum(score_buffer))/float(len(score_buffer)) if len(score_buffer) >= 100 else float(0)

    rl.write('{} {} {} {} {} {} {}\n'.format(episode,avscore,l100_score,minscore,maxscore,rm_size,(epsilon if episode > WARMUP_EPISODES else '-')))
    rl.flush()
    print("Episode: {}; Score: {} ({}); min.Score: {}; max.Score: {}; RMSize: {}; Epsilon: {}".format(episode,avscore,l100_score,minscore,maxscore,rm_size,(epsilon if episode > WARMUP_EPISODES else '-')))
